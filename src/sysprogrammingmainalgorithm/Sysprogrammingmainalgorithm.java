...
public class Sysprogrammingmainalgorithm {
    private static boolean LL1_check(MyLang lang)
    {
        ...
        while(i$.hasNext()) {
            ...
            if (roole.length > 1)
            {
                first = lang.getLexemaText(roole[0]);
                second = lang.getLexemaText(roole[1]);
                if (map.getOrDefault(first, "default").equals(second))
                {
                    return false;
                }
        ...
        do {
            ...
            switch (codeAction) {
                ...
                case 2: //2. Лабораторна робота студента
                    System.out.println(LL1_check(testLang));
                    break;
